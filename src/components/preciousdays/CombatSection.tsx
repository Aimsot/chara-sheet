...
export const CombatSection: React.FC<CombatSectionProps> = ({ char, setChar, isReadOnly }) => {
  ...
  const updateModifier = (
    ...
    setChar((prev) => {
      const targetGroup = prev[target] as any;
      return {
        ...prev,
        [target]: {
          ...targetGroup,
          [key]: {
            ...(targetGroup[key] || { modifier: 0 }),
            modifier: newValue,
          },
        },
      };
  ...
  // --- 行レンダリング (tableStyles を使用したデザインへ変更) ---
  const renderRow = (
    label: string,
    target: 'combatValues' | 'specialChecks',
    key: string,
        ...
        <div className={tableStyles.cell}>
          {isReadOnly ? (
            ...
            <div className={formStyles.stepper}>
              <button
                onClick={() => updateModifier(target, key, (isInvalid ? 0 : modifier) - 1)}
                type='button'
              >
                -
              </button>
              <NumberInput onChange={(val) => updateModifier(target, key, val)} value={modifier} />
              <button
                onClick={() => updateModifier(target, key, (isInvalid ? 0 : modifier) + 1)}
                type='button'
              >
      ...
      <div className={`${cardStyles.accordionContent} ${!isOpen ? cardStyles.closed : ''}`}>
        ...
        <div className={`${tableStyles.scrollContainer} ${layoutStyles.mt4}`}>
          <div className={tableStyles.gridTable}>
            ...
            {renderRow('鑑定', 'specialChecks', 'appraisal', stats.lore.base, '2d6+知力', true)}
          </div>
        </div>
      </div>
    </section>
